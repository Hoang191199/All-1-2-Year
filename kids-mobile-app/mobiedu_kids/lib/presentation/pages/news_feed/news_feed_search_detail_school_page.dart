...
import 'package:mobiedu_kids/presentation/pages/news_feed/widgets/news_feed_search_school_page_item.dart';
import 'package:mobiedu_kids/presentation/widgets/circular_loading_indicator.dart';
import 'package:mobiedu_kids/presentation/widgets/horizontal_divider_no_padding.dart';

class NewsFeedSearchDetailSchoolPage extends StatelessWidget {
  NewsFeedSearchDetailSchoolPage({super.key});
  ...
  Widget build(BuildContext context) {
    ...
    return GetX(
      ...
      initState: (state) async {
        newsFeedSearchController.searchDetailNewsFeed(newsFeedSearchController.searchTextEditingController.text, TypeSearchDetail.pages, null);
                        ...
                        children: [
                          ...
                          ListView.separated(
                            ...
                            itemCount: newsFeedSearchController.isLoadMore.value 
                              ? newsFeedSearchController.searchDetailPageData.length + 1 
                              : newsFeedSearchController.searchDetailPageData.length,
                            itemBuilder: (context, index) {
                              if (index < newsFeedSearchController.searchDetailPageData.length) {
                                final page = newsFeedSearchController.searchDetailPageData[index];
                                return NewsFeedSearchSchoolPageItem(page: page);
                              } else {
                                return const Center(
                                  child: CircularLoadingIndicator(),
                                );
  ...
  void scrollListener() {
    if (newsFeedSearchController.canLoadMore.value) {
      if (scrollController.position.pixels == scrollController.position.maxScrollExtent) {
        newsFeedSearchController.searchDetailNewsFeedMore(newsFeedSearchController.searchTextEditingController.text, TypeSearchDetail.pages, null);
      }
    }
  }
}